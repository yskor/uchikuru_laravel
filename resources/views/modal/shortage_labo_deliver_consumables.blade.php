			...
			<div id="confirm-{{$data->consumables_code}}" class="modal-content">
				...
				<div class="modal-body">
					...
					<div class="w-100">
						...
						<input type="hidden" name="consumables_category_code" value="{{$data->consumables_category_code}}">
						{{-- 現施設在庫数の入力 --}}
						...
						<div class="input-group mb-2"style="width:220px;">
							...
							<input type="number" class="form-control text-center" id="deliver-number-{{$data->consumables_code}}" name="deliver_number" value="{{ $data->shortage_quantity}}">
						...
						<div id="deliver-number-{{$data->consumables_code}}-result">
                            <p>
								納品数内訳：{{ $data->shortage_quantity}}@if($data->quantity == 1)個 @else 箱@endif
								@if($data->quantity != 1)
									{{-- 消費単位が個数かつ1以上の時 --}}
									({{ $data->quantity * $data->shortage_quantity }}個)
								@endif
							</p>
                        </div>
                        <script>

    ...
    <div id="facilitys">
        ...
        @endif
            @foreach ($facility_all as $facility)
                @if ($office_data->office_code == $facility->office_code and $facility->prefecture_code == 16)
                <a class="btn btn-primary" href="{{route('facility_ship_list', ['office_code' => $facility->office_code])}}" id="search-carehome-facility-code-{{ $facility->facility_name }}" style="width: 80px">{{ $facility->facility_name }}</a>
                @elseif ($facility->prefecture_code == 16)
                <a class="btn btn-outline-primary" href="{{route('facility_ship_list', ['office_code' => $facility->office_code])}}" id="search-carehome-facility-code-{{ $facility->facility_name }}" style="width: 80px">{{ $facility->facility_name }}</a>
                @endif
            @endforeach
        </div>
        ...
        @endif
            @foreach ($facility_all as $facility)
                @if ($office_data->office_code == $facility->office_code and $facility->prefecture_code == 17)
                <a class="btn btn-danger" href="{{route('facility_ship_list', ['office_code' => $facility->office_code])}}" id="search-carehome-facility-code-{{ $facility->facility_name }}" style="width: 80px">{{ $facility->facility_name }}</a>
                ...
                <a class="btn btn-outline-danger" href="{{route('facility_ship_list', ['office_code' => $facility->office_code])}}" id="search-carehome-facility-code-{{ $facility->facility_name }}" style="width: 80px">{{ $facility->facility_name }}</a>
                @endif
            @endforeach
        </div>
    </div>

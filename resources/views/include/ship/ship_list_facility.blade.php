    ...
    <div id="facilitys">
        <div class="input-group w-100 mb-1" id="toyama-area">
            @foreach ($facility_all as $facility)
                ...
                <a class="btn btn-primary" style="width: 80px" href="{{route('facility_ship_list', ['office_code' => $facility->office_code])}}" id="search-carehome-facility-code-{{ $facility->facility_name }}">{{ $facility->facility_name }}</a>
                @elseif ($facility->prefecture_code == 16)
                <a class="btn btn-outline-primary" style="width: 80px" href="{{route('facility_ship_list', ['office_code' => $facility->office_code])}}" id="search-carehome-facility-code-{{ $facility->facility_name }}">{{ $facility->facility_name }}</a>
                @endif
            @endforeach
        </div>

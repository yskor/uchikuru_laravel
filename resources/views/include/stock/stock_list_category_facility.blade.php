    ...
    <div id="facilitys">
        <div class="d-flex mb-2">
            ...
            <a class="btn btn-{{$outline}}secondary" style="width: 80px"
                href="{{route('facility_category_stock_list', ['office_code'=>90, 'consumables_category_code'=>$consumables_category_code])}}"
                id="search-carehome-facility-code-LABO">LABO</a>
            @else
            <a class="btn btn-{{$outline}}secondary" style="width: 80px"
                href="{{route('facility_category_stock_list', ['office_code'=>91, 'consumables_category_code'=>$consumables_category_code])}}"
                id="search-carehome-facility-code-本部">本部</a>
            @endif
            {{-- 富山の施設を選択した場合 --}}
            @if($office_data->prefecture_code == 16 or $office_code == 91)
            <div class="input-group w-100" id="toyama-area">
                ...
                <div class="mt-2">
                    <div class="input-group">
                        <form class="d-flex"
                            action="{{ route('facility_category_stock_list_search', ['office_code' => $office_code, 'consumables_category_code' => $consumables_category_code])}}"
                            method="get">
                            @csrf
                            <div class="form-outline" style="">
                                <input type="search" id="keyword" name="keyword" class="form-control"
                                    placeholder="キーワード(消耗品の名前)" value="{{$search_name}}">
                            </div>
